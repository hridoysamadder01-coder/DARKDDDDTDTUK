...
const CHECKS: Check[] = [
  ...
  { label: 'ACCESS SIGNATURE', value: 'VERIFIED', kind: 'ok' },
  { label: 'CLEARANCE LEVEL', value: 'RESTRICTED', kind: 'warn' },
]
...
export default function BootSequence({ onDone }: { onDone: () => void }) {
  ...
  const intro = useTypewriter('INITIALIZING SECURE TERMINAL...', {
    speed: 42,
    onChar: () => play('key'),
  ...
  const steps: SequenceStep[] = useMemo(() => {
    ...
    s.push({ at: 1150, run: () => { setPhase('override'); play('glitch') } })
    s.push({ at: 1050, run: () => { setPhase('out'); play('confirm') } })
    s.push({ at: 620, run: () => onDone() })
    return s
      ...
      <div className="term">
        ...
        <AnimatePresence>
          {phase === 'alert' && (
            ...
            >
              <GlitchText text="UNAUTHORIZED ACCESS DETECTED" always />
            </motion.div>
          )}
        </AnimatePresence>
